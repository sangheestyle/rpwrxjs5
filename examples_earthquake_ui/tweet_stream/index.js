...
function onConnect(ws) {
  ...
  Observable.fromEvent(stream, 'tweet').subscribe(function(tweetObject) {
  ...
  /*
  var onMessage = Observable.fromEvent(ws, 'message')
  ...
  Rx.Observable
    ...
    .scan([], function(boundsArray, quake) { //(1)
      var bounds = [ //(2)
        ...
        coordinate = coordinate.function toString() { [native code] }();
    ...
    .subscribe(function(boundsArray) { //(4)
      stream.stop();
      stream.params.locations = boundsArray.function toString() { [native code] }();
      stream.start();
    });
}
